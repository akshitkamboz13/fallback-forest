...
const Renderer = (() => {
  ...
  // ── DOM ──────────────────────────────────────────────────────────────────
  const defs = mk('defs');
  const skyG = mk('linearGradient', { id: 'sG', x1: '0', y1: '0', x2: '0', y2: '1' });
  const ss0 = mk('stop', { offset: '0%' }), ss1 = mk('stop', { offset: '55%' }), ss2 = mk('stop', { offset: '100%' });
  skyG.append(ss0, ss1, ss2);

  const gndG = mk('linearGradient', { id: 'gG', x1: '0', y1: '0', x2: '0', y2: '1' });
  const gs0 = mk('stop', { offset: '0%' }), gs1 = mk('stop', { offset: '100%' });
  gndG.append(gs0, gs1);
  defs.append(skyG, gndG);
  svg.append(defs);

  const skyR   = mk('rect', { x: 0, y: 0, fill: 'url(#sG)' });
  const starsG = mk('g');
  const celG   = mk('g');
  const gndR   = mk('rect', { x: 0, fill: 'url(#gG)' });
  const mistR  = mk('rect', { x: 0, height: '28' });
  const sceneG = mk('g', { id: 'scene' }); // Unified depth layer for 3D occlusion
  const leafG  = mk('g', { id: 'leaves' });
  const vigR   = mk('rect', { x: 0, y: 0, fill: '#000', opacity: '0.14', 'pointer-events': 'none' });
  svg.append(skyR, starsG, celG, gndR, mistR, sceneG, leafG, vigR);

  // Stars
  Sky.STARS.forEach(s => {
    const c = mk('circle', { cx: s.x + '%', cy: s.y + '%', r: s.r, fill: '#fff', opacity: s.b.toFixed(2) });
    starsG.append(c);
  });

  // Sun / Moon
  const sunGlow = mk('circle', { r: '52', fill: 'rgba(255,238,110,0.13)' });
  const sunEl   = mk('circle', { r: '21', fill: '#fff6b0' });
  const moonEl  = mk('circle', { r: '16', fill: '#dce8fa' });
  ...
  // ── Projection with Earthquake Shake & Head Bob ─────────────────────────
  ...
  function drawTerrainTile(tile, player, py, sky, shakeX = 0, shakeY = 0) {
    ...
    const c = tile.color;
    const baseCol = applyLit(c.hue, c.sat, c.lit, sky);
    const poly = mk('polygon', { points: pts, fill: baseCol, opacity: '1.0' });
    return { poly, cz: avgCz };
  }

  // ── 3D City Skyscraper Building Renderer (Rises from underground) ───────
  function dBuilding(b, p, sky, riseProgress = 1.0) {
    const { sx, sy: base, df } = p;
    const bW = b.w * df, bH = b.h * df * riseProgress; // Scaled by rise factor
    if (bH <= 1) return null;
    ...
    const g = mk('g', { opacity: Math.min(1.0, riseProgress * 1.2).toFixed(2) });

    // Main Facade Rect
    const topY = base - bH;
    g.append(mk('rect', {
      x: (sx - bW * 0.5).toFixed(1),
      y: topY.toFixed(1),
      width: bW.toFixed(1),
      height: bH.toFixed(1),
      fill: col,
    ...
    const rows = b.windowRows, cols = b.windowCols;
    ...
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        ...
        g.append(mk('rect', {
          ...
          height: wH.toFixed(1),
          fill: winCol,
          opacity: (0.55 + ((r + c) % 3) * 0.20).toFixed(2)
        }));
      }
    }
  ...
  function dTallTree(e, p, sky) {
    ...
    const g = mk('g', { opacity: fogA(p.cz).toFixed(3) });
    g.append(
      mk('path', { d: trunkP(sx, base, trH, trW, lean), fill: trC }),
      mk('path', { d: tear(lx, tipY, botY, midY, caW), fill: col }),
      mk('path', { d: 'M' + lx + ',' + botY + ' C' + (lx + caW * .1) + ',' + midY + ' ' + (lx + caW * .1) + ',' + (tipY + caW * .32) + ' ' + lx + ',' + tipY + ' C' + (lx + caW * .52) + ',' + (tipY + caW * .32) + ' ' + (lx + caW * .74) + ',' + midY + ' ' + lx + ',' + botY + 'Z', fill: drk, opacity: '.52' }),
    );
    return g;
  }
  ...
  function dRoundTree(e, p, sky) {
    ...
    g.append(
      mk('path', { d: trunkP(sx, base, trH, trW, lean), fill: trC }),
      mk('path', { d: tear(lx, tipY, botY, midY, caW), fill: col }),
      mk('path', { d: 'M' + lx + ',' + botY + ' C' + (lx + caW * .14) + ',' + midY + ' ' + (lx + caW * .14) + ',' + (tipY + caW * .26) + ' ' + lx + ',' + tipY + ' C' + (lx + caW * .55) + ',' + (tipY + caW * .26) + ' ' + (lx + caW * .76) + ',' + midY + ' ' + lx + ',' + botY + 'Z', fill: drk, opacity: '.46' }),
      mk('path', { d: 'M' + lx + ',' + botY + ' C' + (lx - caW * .62) + ',' + midY + ' ' + (lx - caW * .44) + ',' + (tipY + caW * .26) + ' ' + lx + ',' + tipY + ' C' + lx + ',' + (tipY + caW * .42) + ' ' + (lx - caW * .26) + ',' + midY + ' ' + lx + ',' + botY + 'Z', fill: '#fff', opacity: '.06' }),
    );
    return g;
  }
  ...
  function dPine(e, p, sky) {
    ...
    const n = e.layers, col = applyLit(e.hue, e.sat, e.lit, sky), drk = applyDark(e.hue, e.sat, e.lit, sky);
    const trC = applyLit(25, 38, 11, sky);
    const g = mk('g', { opacity: fogA(p.cz).toFixed(3) });
    g.append(mk('path', { d: trunkP(sx, base, trH, trW, 0), fill: trC }));
    const lH = 3.2 * sc * df / n;
    for (let i = 0; i < n; i++) {
      const t = i / n, w = (2.8 * sc * df) * (1 - t * .52);
      const layBase = base - trH - i * lH * .68, tip = layBase - lH * 1.2;
      g.append(
        mk('path', { d: 'M' + (sx - w) + ',' + layBase + ' L' + (sx + w) + ',' + layBase + ' L' + sx + ',' + tip + 'Z', fill: col }),
        mk('path', { d: 'M' + sx + ',' + layBase + ' L' + (sx + w) + ',' + layBase + ' L' + sx + ',' + tip + 'Z', fill: drk, opacity: '.48' }),
      );
    }
    return g;
  }
  ...
  function dOak(e, p, sky) {
    ...
    const trC = applyLit(25, 38, 11, sky);
    const g = mk('g', { opacity: fogA(p.cz).toFixed(3) });
    g.append(mk('path', { d: trunkP(sx, base, trH, trW, lean), fill: trC }));
    for (const [ox, oy, or] of e.blobO) {
      const bx = (lx + ox * bs * .52).toFixed(1), by = (base - trH - oy * bs * .62).toFixed(1);
      const rx = (or * bs * .72).toFixed(1), ry = (or * bs * .58).toFixed(1);
      g.append(mk('ellipse', { cx: bx, cy: by, rx, ry, fill: col, opacity: '.90' }));
    }
    for (const [ox, oy, or] of e.blobO) {
      if (ox > 0.2) {
        const bx = (lx + ox * bs * .52).toFixed(1), by = (base - trH - oy * bs * .62).toFixed(1);
        const rx = (or * bs * .35).toFixed(1), ry = (or * bs * .48).toFixed(1);
        g.append(mk('ellipse', { cx: bx, cy: by, rx, ry, fill: drk, opacity: '.38' }));
      }
    }
    return g;
  }
  ...
  function dDeadTree(e, p, sky) {
    ...
    const col = applyLit(25, 18, 18, sky), drk = applyLit(25, 14, 10, sky);
    const g = mk('g', { opacity: fogA(p.cz).toFixed(3) });
    g.append(mk('path', { d: trunkP(sx, base, trH, trW, lean), fill: col }));
    for (const [bang, blen, bpos] of e.brA) {
      ...
      const bw = Math.max(1, trW * .52);
      g.append(mk('line', { x1: bsx.toFixed(1), y1: bsy.toFixed(1), x2: bex.toFixed(1), y2: bey.toFixed(1), stroke: col, 'stroke-width': bw }));
      const sbex = bex + Math.cos(bang + .55) * blen * trH * .28;
      const sbey = bey - Math.abs(Math.sin(bang + .55)) * blen * trH * .18;
      g.append(mk('line', { x1: bex.toFixed(1), y1: bey.toFixed(1), x2: sbex.toFixed(1), y2: sbey.toFixed(1), stroke: drk, 'stroke-width': Math.max(.8, bw * .6) }));
    }
    return g;
  }
  ...
  function dBush(e, p, sky) {
    ...
    for (const [ox, oy, or] of e.blobO) {
      ...
      g.append(mk('ellipse', { cx: bx, cy: by, rx, ry, fill: col }));
    }
    for (const [ox, oy, or] of e.blobO) {
      if (ox > 0.25) {
        const bx = (sx + ox * bs * .52).toFixed(1), by = (base - oy * bs * .5).toFixed(1);
        const rx = (or * bs * .32).toFixed(1), ry = (or * bs * .38).toFixed(1);
        g.append(mk('ellipse', { cx: bx, cy: by, rx, ry, fill: drk, opacity: '.42' }));
      }
    }
    return g;
  }
  ...
  function dFern(e, p, sky) {
    const { sx, sy: base, df } = p, sc = e.sc;
    const flen = 2.4 * sc * df, n = e.fronds, spread = e.spread;
    const col = applyLit(e.hue, e.sat, e.lit, sky), drk = applyDark(e.hue, e.sat, e.lit, sky);
    const g = mk('g', { opacity: fogA(p.cz).toFixed(3) });
    for (let i = 0; i < n; i++) {
      const a = ((i / (n - 1)) * 2 - 1) * Math.PI * spread - Math.PI / 2;
      const ex = (sx + Math.cos(a) * flen).toFixed(1), ey = (base + Math.sin(a) * flen).toFixed(1);
      const mx = (sx + Math.cos(a) * .55 * flen - Math.sin(a) * .18 * flen).toFixed(1);
      const my = (base + Math.sin(a) * .55 * flen + Math.cos(a) * .18 * flen).toFixed(1);
      const sw = Math.max(.8, flen * .09);
      g.append(mk('path', { d: 'M' + sx + ',' + base + ' Q' + mx + ',' + my + ' ' + ex + ',' + ey, stroke: i % 2 ? drk : col, 'stroke-width': sw, fill: 'none', 'stroke-linecap': 'round' }));
    }
    return g;
  }
  ...
  function dGrass(e, p, sky) {
    const { sx, sy: base, df } = p, sc = e.sc;
    const blen = 2.2 * sc * df, n = e.blades, spread = 1.6 * sc * df, sway = e.sway;
    const col = applyLit(e.hue, e.sat, e.lit, sky);
    const g = mk('g', { opacity: fogA(p.cz).toFixed(3) });
    for (let i = 0; i < n; i++) {
      const bx = sx + (i / (n - 1) - .5) * spread;
      const tx = (bx + sway * blen * ((i % 3 - 1) * .42)).toFixed(1);
      const sw = Math.max(.8, spread * .10);
      g.append(mk('path', { d: 'M' + bx.toFixed(1) + ',' + base + ' Q' + ((bx + parseFloat(tx)) / 2).toFixed(1) + ',' + (base - blen * .62).toFixed(1) + ' ' + tx + ',' + (base - blen).toFixed(1), stroke: col, 'stroke-width': sw, fill: 'none', 'stroke-linecap': 'round' }));
    }
    return g;
  }
  ...
  function dFlower(e, p, sky) {
    ...
    const g = mk('g', { opacity: fogA(p.cz).toFixed(3) });
    const sw = Math.max(.8, sH * .065);
    g.append(mk('path', { d: 'M' + sx + ',' + base + ' Q' + (sx + sw * 2) + ',' + (base - sH * .62) + ' ' + sx + ',' + (base - sH), stroke: sCol, 'stroke-width': sw, fill: 'none', 'stroke-linecap': 'round' }));
    const n = e.petals;
    for (let i = 0; i < n; i++) {
      const a = (i / n) * Math.PI * 2;
      const px2 = (sx + Math.cos(a) * pr).toFixed(1), py2 = (base - sH + Math.sin(a) * pr * .72).toFixed(1);
      g.append(mk('ellipse', { cx: px2, cy: py2, rx: (pr * .58).toFixed(1), ry: (pr * .42).toFixed(1), fill: pCol }));
    }
    g.append(mk('circle', { cx: sx, cy: (base - sH).toFixed(1), r: (pr * .38).toFixed(1), fill: 'hsl(45,75%,' + clamp(55 * (.08 + sky.amb * .92), 4, 75) + '%)' }));
    return g;
  }
  ...
  function dMushroom(e, p, sky) {
    ...
    const g = mk('g', { opacity: fogA(p.cz).toFixed(3) });
    const sw = Math.max(1.5, cR * .32);
    g.append(mk('rect', { x: (sx - sw / 2).toFixed(1), y: cy, width: sw.toFixed(1), height: sH.toFixed(1), fill: sCol }));
    g.append(mk('ellipse', { cx: sx.toFixed(1), cy, rx: cR.toFixed(1), ry: (cR * .65).toFixed(1), fill: cCol }));
    if (e.spotted) {
      const sR = Math.max(1.2, cR * .2);
      for (let i = 0; i < 4; i++) {
        const a = i * Math.PI / 2 + .35;
        g.append(mk('circle', { cx: (sx + Math.cos(a) * cR * .52).toFixed(1), cy: (base - sH + Math.sin(a) * cR * .32).toFixed(1), r: sR.toFixed(1), fill: 'rgba(255,255,255,0.32)' }));
      }
    }
    g.append(mk('ellipse', { cx: (sx + cR * .22).toFixed(1), cy, rx: (cR * .44).toFixed(1), ry: (cR * .52).toFixed(1), fill: 'hsl(' + e.cHue + ',' + e.cSat * .55 + '%,' + clamp(e.cLit * (.06 + sky.amb * .7) * .48, 2, 38) + '%)', opacity: '.50' }));
    return g;
  }

  function dRock(e, p, sky) {
    const { sx, sy: base, df } = p, sc = e.sc;
    const rx = sc * df * 1.65, ry = rx * e.flat, n = e.sides;
    const pts = [], spts = [];
    for (let i = 0; i < n; i++) {
      const a = (i / n) * Math.PI * 2, j = e.jit[i] || 1;
      const px2 = (sx + Math.cos(a) * rx * j).toFixed(1), py2 = (base - ry * .5 + Math.sin(a) * ry * j * .52).toFixed(1);
      pts.push(px2 + ',' + py2);
      if (i > n / 3 && i < n * 2 / 3) spts.push(px2 + ',' + py2);
    }
    const col = applyLit(20, 8, e.grey, sky), drk = applyDark(20, 6, e.grey, sky);
    const g = mk('g', { opacity: fogA(p.cz).toFixed(3) });
    g.append(mk('polygon', { points: pts.join(' '), fill: col }));
    if (spts.length > 1) g.append(mk('polygon', { points: [...spts, sx + ',' + base].join(' '), fill: drk, opacity: '.38' }));
    return g;
  }
  ...
  function drawLeaves(sky) {
    ...
    for (const p of particles) {
      ...
      const opacity = Math.min(0.88, fogA(pr.cz) * 0.95);

      const pathData = `M ${pr.sx.toFixed(1)} ${(pr.sy - leafH * 0.5).toFixed(1)} Q ${(pr.sx + leafW).toFixed(1)} ${pr.sy.toFixed(1)} ${pr.sx.toFixed(1)} ${(pr.sy + leafH * 0.5).toFixed(1)} Q ${(pr.sx - leafW).toFixed(1)} ${pr.sy.toFixed(1)} ${pr.sx.toFixed(1)} ${(pr.sy - leafH * 0.5).toFixed(1)}`;
      const leafPath = mk('path', {
        d: pathData,
        fill: col,
        opacity: opacity.toFixed(3),
        transform: `rotate(${(p.rot * 57.3).toFixed(1)}, ${pr.sx.toFixed(1)}, ${pr.sy.toFixed(1)})`
      });
  ...
  // ── Render Frame (Supports Earthquake Shake, Sinking Trees & Rising City) ──
  function render(player, worldData, sky, transitionState = 'FOREST', transitionProgress = 0.0) {
    ...
    // Calculate camera shake offsets during earthquake
    ...
    // 1. Project 3D ground tiles
    for (const tile of worldData.tiles) {
      const res = drawTerrainTile(tile, player, py, sky, shakeX, shakeY);
      if (res) sceneItems.push({ poly: res.poly, cz: res.cz });
    }

    // 2. Project forest entities (Trees sink underground when transitionProgress > 0)
    const sinkAmount = (transitionState === 'TRANSITION' || transitionState === 'EARTHQUAKE') ? transitionProgress * 18.0 : 0.0;
    for (const e of worldData.ents) {
    ...
    // 3. Project city skyscrapers (Rise UP from underground when transitionProgress > 0)
    ...
    // 4. Sort ALL elements together by depth (far to near) for proper 3D occlusion
    sceneItems.sort((a, b) => b.cz - a.cz);
    for (const item of sceneItems) {
      sceneG.append(item.poly);
    }

    // 5. Render flying leaves in wind
    drawLeaves(sky);
  }
