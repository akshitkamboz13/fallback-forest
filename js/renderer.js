...
const Renderer = (() => {
  ...
  if (!svg) console.error('SVG #canvas element not found!');
  ...
  // ── 3D City Skyscraper Building Renderer ─────────────────────────────────
  ...
  function drawLeaves(sky) {
    leafG.textContent = ''; // Fix leaf tracking lines bug by clearing leafG per frame!
    ...
    for (let i = 0; i < particles.length; i++) {
      ...
      const opacity = Math.min(0.85, fogA(pr.cz) * 0.95);
  ...
  function render(player, worldData, sky, transitionState = 'FOREST', transitionProgress = 0.0) {
    ...
    sceneG.textContent = ''; // Clear main 3D scene layer
